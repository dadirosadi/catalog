...
const ProductDetailPage = ({ relatedProduct, productDetail, addItem, ...props }) => {
    return <>
        {
            productDetail && relatedProduct.length ? <div className='collections-overview'>
                ...
                <div className='info'>
                    ...
                    <h3 className='more' onClick={() => { props.history.push(`/${relatedProduct[0][0].routeName.toLowerCase()}`) }}>See More</h3>
                </div>

                {
                    relatedProduct[0].map(({ id, ...otherCollectionProps }) => (
